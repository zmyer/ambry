...
class PersistentIndex {
    ...
    */
    ...
    PersistentIndex(String datadir, String storeId, ScheduledExecutorService scheduler, Log log, StoreConfig config,
        ...
        try {
            for (int i = 0; i < indexFiles.size(); i++) {
                ...
                //根据获取到的索引文件，创建具体的indexSegment对象
            ...
            //设置每个logSegment的末尾偏移量
            setEndOffsets();
            //设置当前log文件可读写的logSegment对象
            log.setActiveSegment(getCurrentEndOffset().getName());
            //设置启动时的log文件对应的可读写的偏移量
            ...
            if (hardDelete != null) {
                ...
                //创建hard删除对象
                hardDeleter = new HardDeleter(config, metrics, datadir, log, this, hardDelete, factory, diskIOScheduler,
                        time);
    ...
    private void recover(MessageStoreRecovery recovery) throws StoreException, IOException {
        ...
        //根据恢复起始偏移量，查找具体的需要恢复的logSegment对象
        ...
        while (logSegmentToRecover != null) {
            ...
            for (MessageInfo info : messagesRecovered) {
                ...
                if (info.isDeleted()) {
                    ...
                    //根据恢复的消息信息，创建具体的索引value
                    IndexValue newValue =
                            new IndexValue(info.getSize(), runningOffset, info.getExpirationTimeInMs(),
                                    info.getOperationTimeMs(),
                                    info.getAccountId(), info.getContainerId());
                    //将索引value对象插入到indexSegment中
                    addToIndex(new IndexEntry(info.getStoreKey(), newValue, null),
                            new FileSpan(runningOffset, infoEndOffset));
    ...
    private void setEndOffsets() throws IOException {
        for (Offset segmentStartOffset : validIndexSegments.keySet()) {
            //根据segment的起始偏移量，获取下一个indexSegment的起始偏移量
    ...
    void changeIndexSegments(List<File> segmentFilesToAdd, Set<Offset> segmentsToRemove) throws StoreException {
        ...
        for (File indexSegmentFile : segmentFilesToAdd) {
            //根据待添加的文件对象，创建具体的indexSegment对象
    ...
    void addToIndex(IndexEntry entry, FileSpan fileSpan) throws StoreException {
        ...
        if (needToRollOverIndex(entry)) {
            ...
            //创建新的indexSegment对象
            IndexSegment info =
                    new IndexSegment(dataDir, entry.getValue().getOffset(), factory, entrySize, valueSize, config,
                            metrics, time);
            //将索引项插入到indexSegment对象中
    ...
    private boolean needToRollOverIndex(IndexEntry entry) {
        ...
        //如果当前的segment写入量已经超过了最大值，则需要滚动
        if (lastSegment.getSizeWritten() >= maxInMemoryIndexSizeInBytes) {
            logger.info(
                    "Index: {} Rolling over from {} to {} because the size written {} >= maxInMemoryIndexSizeInBytes {}",
                    dataDir, lastSegment.getStartOffset(), offset, lastSegment.getSizeWritten(),
                    maxInMemoryIndexSizeInBytes);
            return true;
        }
        //如果当前的segment包含的索引数目超过了最大值，则需要滚动
        if (lastSegment.getNumberOfItems() >= maxInMemoryNumElements) {
            logger.info(
                    ...
                    dataDir, lastSegment.getStartOffset(), offset, lastSegment.getNumberOfItems(),
                    maxInMemoryNumElements);
            return true;
        }
        //如果segment持久化实例的大小小于当前实体的大小，也需要滚动
    ...
    private IndexValue findKey(StoreKey key, FileSpan fileSpan, IndexEntryType type,
        ...
        try {
            ...
            if (fileSpan == null) {
                ...
                //如果查找文件范围为空，则默认是全部indexSegment
                segmentsMapToSearch = indexSegments.descendingMap();
            } else {
                logger.trace(
                        "Searching for " + key + " in index with filespan ranging from " + fileSpan.getStartOffset() +
                                " to "
                                + fileSpan.getEndOffset());
                //否则是根据查找文件范围，来锁定具体需要查找的indexSegment
            ...
            for (Map.Entry<Offset, IndexSegment> entry : segmentsMapToSearch.entrySet()) {
                ...
                if (value != null) {
                    logger.trace("Index : {} found value offset {} size {} ttl {}", dataDir, value.getOffset(),
                            value.getSize(),
    ...
    private IndexValue markAsDeleted(StoreKey id, FileSpan fileSpan, MessageInfo info, long deletionTimeMs)
        ...
        if (value == null) {
            ...
            //如果未查找到，则需要新建索引value对象
            newValue =
                    new IndexValue(size, fileSpan.getStartOffset(), info.getExpirationTimeInMs(),
                            info.getOperationTimeMs(),
                            info.getAccountId(), info.getContainerId());
            newValue.clearOriginalMessageOffset();
        } else {
            //如果存在，则需要根据查找的结果，重新构造索引value
    ...
    BlobReadOptions getBlobReadInfo(StoreKey id, EnumSet<StoreGetOptions> getOptions) throws StoreException {
        ...
        if (value == null) {
            ...
            if (!getOptions.contains(StoreGetOptions.Store_Include_Deleted)) {
                throw new StoreException("Id " + id + " has been deleted in index " + dataDir,
            ...
            readOptions = new BlobReadOptions(log, value.getOffset(),
                    new MessageInfo(id, value.getSize(), value.isFlagSet(IndexValue.Flags.Delete_Index),
                            value.getExpiresAtMs(),
                            journal.getCrcOfKey(id), value.getAccountId(), value.getContainerId(),
        ...
        try {
            ...
            if (value.getOriginalMessageOffset() != value.getOffset().getOffset()
                ...
                MessageInfo deletedBlobInfo =
                        hardDelete.getMessageInfo(log.getSegment(logSegmentName), value.getOriginalMessageOffset(),
                                factory);
                if (putValue != null && putValue.getOffset().getName().equals(value.getOffset().getName())) {
                    ...
                    long deletedBlobInfoExpiresAtMs = Utils.getTimeInMsToTheNearestSec(
                            deletedBlobInfo.getExpirationTimeInMs());
                    if (putValueExpiresAtMs != deletedBlobInfoExpiresAtMs) {
                        logger.error(
                                "Expire time in PUT index entry {} is different from that in the PUT record {} for ID {}",
                ...
                throw new StoreException("Did not find PUT index entry for key [" + key
                        + "] and the the original offset in value of the DELETE entry was [" +
        ...
        try {
            ...
            if (!storeToken.getType().equals(StoreFindToken.Type.IndexBased)) {
                ...
                if (entries != null && offsetToStart.compareTo(journal.getFirstOffset()) >= 0) {
                    startTimeInMs = time.milliseconds();
                    logger.trace(
                            "Index : " + dataDir + " retrieving from journal from offset " + offsetToStart +
                                    " total entries "
                                    + entries.size());
                    //记录当前的起始偏移量
                    ...
                    for (JournalEntry entry : entries) {
                        //根据日志实例对象，构造对应的索引value对象
                        IndexValue value =
                                findKey(entry.getKey(),
                                        new FileSpan(entry.getOffset(), getCurrentEndOffset(indexSegments)),
                                        IndexEntryType.ANY, indexSegments);
                        //通过索引Value对象创建messageInfo,并插入到结果集中
                        messageEntries.add(
                                new MessageInfo(entry.getKey(), value.getSize(),
                ...
                //获取查找token
                StoreFindToken newToken =
                        findEntriesFromSegmentStartOffset(storeToken.getOffset(), storeToken.getStoreKey(),
                                messageEntries,
                                new FindEntriesCondition(maxTotalSizeOfEntries), indexSegments);
        ...
        if (!storeToken.getType().equals(StoreFindToken.Type.Uninitialized) && remoteIncarnationId != null
            ...
            if (!cleanShutdown) {
                ...
                if (!storeToken.getType().equals(StoreFindToken.Type.Uninitialized)
                    ...
                    logger.info(
                            "Index : " + dataDir + " resetting offset after not clean shutdown " + logEndOffsetOnStartup
     ...
     */
    // TODO: 2018/3/22 by zmyer
    private long getTotalBytesRead(StoreFindToken token, List<MessageInfo> messageEntries,
        ...
        if (token.getType().equals(StoreFindToken.Type.IndexBased)) {
            //如果本次查找是依靠索引的话，直接通过偏移量来计算读取的字节数
    ...
    private long getAbsolutePositionInLogForOffset(Offset offset,
        ...
        //统计当前的logSegment对应了多少个indexSegment数目
        int numPrecedingLogSegments = getLogSegmentToIndexSegmentMapping(indexSegments).headMap(offset.getName())
                .size();
        //根据indexSegment的数目计算出具体的字节数，并返回
    ...
    private StoreFindToken findEntriesFromSegmentStartOffset(Offset initialSegmentStartOffset, StoreKey key,
        ...
        //如果待查找偏移量是当前的indexSegment的末尾，则不需要查找，直接报错
    ...
    /* First, get keys from the segment corresponding to the passed in segment start offset if the token has a non-null
       ...
       first. */
        if (key != null) {
            if (segmentToProcess.getEntriesSince(key, findEntriesCondition, messageEntries,
        ...
        while (findEntriesCondition.proceed(currentTotalSizeOfEntries.get(),
            ...
            if (entries != null) {
                ...
                for (JournalEntry entry : entries) {
                    ...
                    IndexValue value =
                            findKey(entry.getKey(), new FileSpan(entry.getOffset(), endOffsetOfSnapshot),
                                    IndexEntryType.ANY,
                                    indexSegments);
                    messageEntries.add(
                            new MessageInfo(entry.getKey(), value.getSize(),
         ...
         * */
                throw new IllegalStateException(
                        "Index : " + dataDir + " findEntriesFromOffset segment start offset " + segmentStartOffset
                                + " is of the latest segment and not found in journal with range [" +
                                journalFirstOffsetBeforeCheck
                                + ", " + journalLastOffsetBeforeCheck + "]");
            } else {
                // Read and populate from the first key in the segment with this segmentStartOffset
                if (segmentToProcess.getEntriesSince(null, findEntriesCondition, messageEntries,
                        currentTotalSizeOfEntries)) {
                    newTokenSegmentStartOffset = segmentStartOffset;
                }
        ...
        if (newTokenOffsetInJournal != null) {
            ...
            throw new IllegalStateException(
                    "Message entries cannot be null. At least one entry should have been returned, start offset: "
                            + initialSegmentStartOffset + ", key: " + key + ", findEntriesCondition: " +
                            findEntriesCondition);
        } else {
            ...
            return newTokenSegmentStartOffset == null ? new StoreFindToken()
                    : new StoreFindToken(messageEntries.get(messageEntries.size() - 1).getStoreKey(),
    ...
    private void validateFileSpan(FileSpan fileSpan, boolean checkWithinSingleSegment) {
        ...
        if (checkWithinSingleSegment && !fileSpan.getStartOffset().getName().equals(
                fileSpan.getEndOffset().getName())) {
            throw new IllegalArgumentException(
    ...
    FindInfo findDeletedEntriesSince(FindToken token, long maxTotalSizeOfEntries, long endTimeSeconds)
        ...
        try {
            ...
            if (storeToken.getType().equals(StoreFindToken.Type.IndexBased)) {
                ...
                // Find the index segment corresponding to the token indexStartOffset.
                // Get entries starting from the token Key in this index.
                newToken = findEntriesFromSegmentStartOffset(storeToken.getOffset(), storeToken.getStoreKey(),
                        messageEntries,
                ...
                if (entries != null) {
                    ...
                    for (JournalEntry entry : entries) {
                        ...
                        if (value.isFlagSet(IndexValue.Flags.Delete_Index)) {
                            messageEntries.add(
                                    new MessageInfo(entry.getKey(), value.getSize(), true, value.getExpiresAtMs(),
                                            value.getAccountId(),
                ...
                if (lastEntry != null) {
                    ...
                    if (logEndOffsetBeforeFlush.compareTo(indexEndOffsetBeforeFlush) < 0) {
                        throw new StoreException(
                                "LogEndOffset " + logEndOffsetBeforeFlush + " before flush cannot be less than "
                                        + "currentEndOffSet of index " + indexEndOffsetBeforeFlush,
                    ...
                    while (prevInfo != null && !prevInfo.isMapped()) {
                        if (prevInfo.getEndOffset().compareTo(currentLogEndPointer) > 0) {
                            String message =
                                    "The read only index cannot have a file end pointer " + prevInfo.getEndOffset()
                    ...
                    for (int i = prevInfosToWrite.size() - 1; i >= 0; i--) {
                        ...
                        logger.trace("Index : {} writing prev index with end offset {}", dataDir,
                                toWrite.getEndOffset());
