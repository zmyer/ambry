...
public class TransmissionFactory {
    ...
    public static Transmission getTransmission(String connectionId, SocketChannel socketChannel, SelectionKey key,
            String remoteHost, int remotePort, Time time, NetworkMetrics metrics, PortType portType,
        ...
        if (portType == PortType.PLAINTEXT) {
            ...
            try {
                //创建SSL传输对象
                sslTransmission =
                        new SSLTransmission(sslFactory, connectionId, socketChannel, key, remoteHost, remotePort, time,
                                metrics,
                                mode);
