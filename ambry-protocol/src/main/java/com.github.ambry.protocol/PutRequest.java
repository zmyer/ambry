...
public class PutRequest extends RequestOrResponse {
    ...
    private int sizeExcludingBlobAndCrcSize() {
        ...
        if (sizeExcludingBlobAndCrc == -1) {
            sizeExcludingBlobAndCrc =
                    (int) super.sizeInBytes() + blobId.sizeInBytes() + BlobPropertiesSerDe.getBlobPropertiesSerDeSize(
                            properties)
                            + USERMETADATA_SIZE_IN_BYTES + usermetadata.capacity() + BLOB_SIZE_IN_BYTES +
                            BLOBTYPE_SIZE_IN_BYTES;
    ...
    public long writeTo(WritableByteChannel channel) throws IOException {
        ...
        if (sentBytes < sizeInBytes()) {
            if (bufferToSend == null) {
                ...
                crc.update(bufferToSend.array(), bufferToSend.arrayOffset() + crcStart,
                        bufferToSend.position() - crcStart);
    ...
    public static class ReceivedPutRequest {
        ...
        ReceivedPutRequest(int correlationId, String clientId, BlobId blobId, BlobProperties blobProperties,
                ByteBuffer userMetadata, long blobSize, BlobType blobType, ByteBuffer blobEncryptionKey,
